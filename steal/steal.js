...
(function() {
	...
	steal.fn = steal.prototype = {
		...
		run: function() {
			...
			// Bust dat cache!
			if ( steal.options.ver != false ) {
				 options.src = options.src + "?v=" + steal.options.ver;
			}
	...
	extend(steal, {
		...
		setScriptOptions: function() {
			...
			if ( scriptOptions ) {
				...
				if ( scriptOptions.indexOf('=') > -1 ) {
					...
					if ( commaSplit[2] !== undefined ) {
						steal.options.ver = commaSplit[2];
					} else {
						steal.options.ver = false;
					}
				}
		...
		createLink: function( location, options ) {
			...
			// Bust dat cache!
			if ( steal.options.ver != false ) {
				 link.href = link.href + "?v=" + steal.options.ver;
			}
